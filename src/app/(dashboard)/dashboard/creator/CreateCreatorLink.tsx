...
import { uploadCreatorLink } from "@/actions/creator.action";
import { toast } from "sonner";

const serverURL: string = "https://lemonclick.vercel.app";

const CreateCreatorLink = () => {
  ...
  const handelUpload = async () => {
    setLoading(true);
    try {
      const res: any = await uploadCreatorLink({ link, name });
      //   console.log(res);

      if (res.upload == null) {
        setValidLink(res.isValidURL);
      } else {
        setURL(`${serverURL}/${res.upload.username}/${res.upload.name}`);
        setValidLink(res.isValidURL);
        setIsVisible(true);
      }
    } catch (error) {
    ...
    <div className="flex items-center space-x-2">
      <Dialog>
        ...
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>Create Creator Link</DialogTitle>
            <DialogDescription>
              Enter the URL and name to create a custom link.
            </DialogDescription>
          </DialogHeader>
...
export default CreateCreatorLink;
